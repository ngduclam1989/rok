...
| **B3** | **Ấn màn hình & Khởi đầu**    | [runtime.py](file:///e:/bot%20rok/automation-farm-rise-of-kingdom/core/bot/runtime.py)                                                                                                                                                                                                                                                                                                          | tap giữa `(1200, 540)` bỏ qua pop-up → chờ ngẫu nhiên từ `delay_after_popup_min` đến `delay_after_popup_max` (mặc định **5–15s**) → gọi `_claim_vip_and_prepare_world()` để chuẩn bị đưa game về WORLD (các hành động VIP và việc vặt liên minh đã được bỏ khỏi vòng chạy tự động).                                                                                                                                                                                                                                                                                                                                                                                                                                    |
...
### 🔹 B3 — Đưa game về WORLD & Nhận VIP tự động nếu bật

- **Vị trí:** `_claim_vip_and_prepare_world()` (khi khởi đầu) và `_handle_queue_full()` (khi hàng chờ đầy) tại [runtime.py](file:///e:/bot%20rok/automation-farm-rise-of-kingdom/core/bot/runtime.py).
- **Quy trình thực hiện:**
  - Mặc định, tính năng nhận VIP tự động được tắt (`enable_vip_claim: false`) để tập trung hoàn toàn vào farm tự động siêu tốc.
  - Khi được bật (`enable_vip_claim: true` trong `devices.yaml` hoặc qua flag `--enable-vip-claim` trên CLI):
    - Để giả lập hành vi giống người thật nhất và tránh bị phát hiện (anti-detect), bot sẽ **quyết định ngẫu nhiên (random choice 50/50)** thứ tự ưu tiên giữa **Nhận VIP** và **Farm**:
      - **Trường hợp 1: Nhận VIP trước, Farm sau**: Thực hiện nhận VIP ngay từ đầu trước khi gửi quân đi farm.
      - **Trường hợp 2: Farm trước, Nhận VIP sau**: Gửi quân đi farm ngay lập tức. Sau khi hàng chờ đầy (`dispatched_count >= MAX_SLOTS`), trước khi chuyển tài khoản, bot mới tiến hành nhận VIP.
    - Nhận VIP (`_claim_vip`): Vào CITY -> click VIP (tọa độ lấy ngẫu nhiên quanh vùng tâm `151,81,236,110`) -> nhận điểm VIP -> nhận rương VIP miễn phí -> BACK.
    - Sau khi xong, đưa game về màn hình **WORLD** sẵn sàng farm.
  - Ngoài ra, bạn vẫn có thể chạy nhận VIP thủ công bất kỳ lúc nào thông qua tham số `--only-claim-vip`.
...
Sau khi chạy bước 5. chờ 2h+-10' và chạy lại bot từ bước 1
...
defaults:
  ...
  enable_vip_claim: true   # Bật/tắt nhận VIP tự động xen kẽ khi chạy farm
...
# Đơn máy kèm bật nhận VIP tự động xen kẽ
python main.py bot --serial 127.0.0.1:5615 --enable-vip-claim

# CHỈ CHẠY NHẬN VIP (rồi dừng lại ngay)
python main.py bot --serial 127.0.0.1:5615 --only-claim-vip
